...
namespace Program
{
    ...
    public class ProgramCombineFiles
    {
        ...
        /// <summary>
        /// Moves mgf files from a directory and all subdirectories. 
        /// </summary>
        public void GrabAllMgfFiles(string pathToDirectory, string destinationDirectory)
        {
            string[] mgfFiles = Directory.GetFiles(pathToDirectory, "*mgf", SearchOption.AllDirectories);
            foreach (string mgfFile in mgfFiles)
            {
                File.Move(mgfFile, Path.Combine(destinationDirectory,Path.GetFileName(mgfFile)));
            }
        }
